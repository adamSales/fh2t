...
for(i in 1:(nlevs-1))
  for(j in (i+1):nlevs){
    ...
    ov <- c(ov,1-mean(diffDat[["hasDelayed"]]))
    diff <- c(diff,
               mean(subset(diffDat,Z==trtLevs[i])[["hasDelayed"]])-
               mean(subset(diffDat,Z==trtLevs[j])[["hasDelayed"]])
             )
...
with(wwc,
  plot(Overall,Differential1,type="l",ylim=c(0,11),
       main="Delayed Including Pretest No-Shows",
