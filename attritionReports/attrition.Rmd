...
att <- dat%>%
    group_by(Z)%>%
    summarize(att=1-mean(hasMidtest))%>%
    ungroup()
diff <- outer(att$att,att$att,FUN='-')
rownames(diff) <- colnames(diff) <- att$Z
...
```{r}
ddd <- dat%>%
    filter(hasMidtest,!is.na(Z), !endsWith(Z,"Resource"))
...
xBalance version:
```{r}
bals <- xbalMult(ff,ddd,trtLevs=unique(ddd$Z)[!endsWith(unique(ddd$Z),'Resource')],strata=list(cls=~ddd$initial_teacher_class),na.rm=TRUE)
plotXbals(bals)
...
diff <- outer(att$att,att$att,FUN='-')
rownames(diff) <- colnames(diff) <- att$Z
kable(diff,digits=2,caption='mid-test differential attrition, excluding pre-test no-shows')
```
...
```{r}
ddd <- dat%>%
    filter(hasMidtest,hasPretest,!is.na(Z), !endsWith(Z,"Resource"))
...
att <- dat%>%
    group_by(Z)%>%
    summarize(att=1-mean(hasPosttest))%>%
    ungroup()
diff <- outer(att$att,att$att,FUN='-')
rownames(diff) <- colnames(diff) <- att$Z
kable(diff,digits=2,caption='post-test differential attrition, including pre-test no-shows')
```
...
```{r}
ddd <- dat%>%
    filter(hasPosttest,!is.na(Z), !endsWith(Z,"Resource"))
...
diff <- outer(att$att,att$att,FUN='-')
rownames(diff) <- colnames(diff) <- att$Z
kable(diff,digits=2,caption='post-test differential attrition, excluding pre-test no-shows')
```
...
```{r}
ddd <- dat%>%
    filter(hasPosttest,hasPretest,!is.na(Z), !endsWith(Z,"Resource"))
...
```{r}
ddd <- dat%>%
    filter(hasDelayed,hasPretest,!is.na(Z), !endsWith(Z,"Resource"))
