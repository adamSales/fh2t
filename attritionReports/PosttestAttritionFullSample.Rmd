...
#Drop schools with <0.01 pretst scores (S03, Liberty)
```{r}
...
schoolDrop <- dat%>%
   group_by(initial_school_id)%>%
    summarize(prePer=mean(hasPretest))%>%
    filter(prePer<0.1)


### virtual kids from S03
teach03 <- read_csv('../data/IES_school_teacher_ID_list_opt_out - teacher (1).csv')%>%
    filter(note%in%schoolDrop$initial_school_id)%>%pull(teacher_id)

dat <- dat%>%
    filter(!initial_school_id%in%schoolDrop$initial_school_id,!initial_teacher_id%in%teach03)
...
for(i in 1:(nlevs-1))
  for(j in (i+1):nlevs){
    ...
    ov <- c(ov,1-mean(diffDat[["hasPosttest"]]))
    diff <- c(diff,
               mean(subset(diffDat,Z==trtLevs[i])[["hasPosttest"]])-
               mean(subset(diffDat,Z==trtLevs[j])[["hasPosttest"]])
             )
...
with(wwc,
  plot(Overall,Differential1,type="l",ylim=c(0,11),
       main="Posttest Including Pretest No-Shows",
