...
Among this subgroup, `r sum(!dat$preMis&!dat$midMis)` also took the
mid-test, so attrition is much lower: `r round(mean(dat$midMis[!dat$preMis])*100)`%. 
...
```{r growth}
t.test(smallDat$mid.total_math_score-smallDat$pre.total_math_score)
```
...
gf_boxplot(mid.total_math_score~condition,data=finalDat,outlier.shape=NA)+geom_jitter()+labs(y='Mid. Total Math Score',x=NULL)+
...
finalDat%>%group_by(condition)%>%mutate(nt=sum(!is.na(mid.total_math_score)))%>%group_by(condition,mid.total_math_score)%>%summarize(percent=n()/nt)%>%ggplot(aes(mid.total_math_score,percent,color=condition,group=condition))+geom_point()+geom_line()+scale_x_continuous(breaks=0:10)+scale_y_continuous(labels=scales::percent,limits=c(0,.15))
```
...
gf_jitter(mid.total_math_score~pre.total_math_score,color=~condition,data=finalDat,size=.5)+geom_smooth(method='lm')+
...
mod1 <- lm_robust(mid.total_math_score~condition,data=finalDat)
mod2 <- lm_robust(mid.total_math_score~condition,data=finalDat,fixed_effects=~initial_teacher_class)
mod4 <- lm_robust(mid.total_math_score~condition+pretest+ESOL,data=finalDat,fixed_effects=~initial_teacher_class)
mod6 <- lm_robust(mid.total_math_score~condition+pretest+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race+ScaleScoreMISS+ESOLMISS+raceMISS,data=finalDat,fixed_effects=~initial_teacher_class)
mod6lm <- lm(mid.total_math_score~condition+pretest+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race+ScaleScoreMISS+ESOLMISS+raceMISS+initial_teacher_class,data=finalDat)
mod7a <- lm_robust(mid.total_math_score~condition*pretest+ESOL,data=finalDat,fixed_effects=~initial_teacher_class)
mod7 <- lm_robust(mid.total_math_score~condition+pretest+condition:pretest+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race+ScaleScoreMISS+ESOLMISS+raceMISS,data=finalDat,fixed_effects=~initial_teacher_class)
mod7lm <- update(mod6lm,.~.+pretest:condition)
mod7neg<- lm_robust(mid.total_math_score~condition+pretestNeg+condition:pretestNeg+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race+ScaleScoreMISS+ESOLMISS+raceMISS,data=finalDat,fixed_effects=~initial_teacher_class)
mod7pos<- lm_robust(mid.total_math_score~condition+pretestPos+condition:pretestPos+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race+ScaleScoreMISS+ESOLMISS+raceMISS,data=finalDat,fixed_effects=~initial_teacher_class)

texreg(list(mod2,mod4,mod6,mod7a,mod7),caption="OLS models estimating the effect of assignment to the instant feedback condition versus the delayed feedback condition. All models except 11 include fixed-effects for classroom; models 3 \\& 4 only include students with pretest scores. Confidence intervals are in brackets under coefficient estimates.",label='tab:ols',stars=c(0.1,0.05,0.01,0.001),include.ci=FALSE)
```

```{r confint}
map(list(mod2,mod4,mod6), confint,parm='conditionInstant')
map(list(mod7a,mod7),confint,parm=c('conditionInstant','conditionInstant:pretest'))
```
...
The model predicts an effect of `r round(mod7neg$coefficients['conditionInstant'],3)`, (CI: [`r round(mod7neg$conf.low[1],3)`,`r round(mod7neg$conf.high[1],3)`]) for subjects with pretest scores of `r unique(na.omit(dat$pre.total_math_score[dat$pretestNeg==0]))`, roughly 1 standard deviation below the mean and 
`r round(mod7pos$coefficients['conditionInstant'],3)`, (CI: [`r round(mod7pos$conf.low[1],3)`,`r round(mod7pos$conf.high[1],3)`]) for subjects with pretest scores of `r unique(na.omit(dat$pre.total_math_score[dat$pretestPos==0]))`, roughly 1 standard deviation above the mean.


```{r regTableForWord,include=FALSE}
if(require(modelsummary))
    modelsummary(list(mod2,mod4,mod6,mod7a,mod7),title="OLS models estimating the effect of assignment to the instant feedback condition versus the delayed feedback condition. All models except 11 include fixed-effects for classroom; models 3 \\& 4 only include students with pretest scores. Confidence intervals are in brackets under coefficient estimates.",output="regressionTable.docx",stars=TRUE)
```
