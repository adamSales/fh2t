...
Randomization was blocked within the `r n_distinct(dat$initial_SectionNumber)` classrooms.
...
tally(midMis~condition,data=dat,format="percent")%>%
    kbl(booktabs=TRUE,caption="Attrition by treatment group.")
```
...
balMid <- xBalance(!midMis~ScaleScore+pre.total_math_score+EIP+ESOL+IEP+FEMALE+GIFTED+student_hispanicEthnicity+race,data=dat,
                   report='all',strata=list(classroom=~initial_SectionNumber))
...
mod2 <- lm_robust(mid.total_math_score~condition,data=dat,fixed_effects=~initial_SectionNumber)
mod4 <- lm_robust(mid.total_math_score~condition+pretest,data=dat,fixed_effects=~initial_SectionNumber)
mod6 <- lm_robust(mid.total_math_score~condition+pretest+ScaleScore+EIP+ESOL+IEP+FEMALE+GIFTED+race,data=dat,fixed_effects=~initial_SectionNumber)
