...
Figure 1 superimposes the results in Table 2, for active control comparisons, onto Figure II.2 from the What Works Clearninghouse (WWC) Standards Handbook, Version 4.1 (WWC 2020), showing that attrition is tolerable under optimistic and/or cautious assumptions.
...
for(i in 1:3)
  for(j in (i+1):4){
    if('ac'%in%colnames(diffAtt)[c(i,j)]){
...
prop.test(Ncond-ncond,Ncond)
```

### Atttrition rates for students in Virtual or In-Person Conditions
...
kable(tab,digits=1,caption="Table 3: Population and sample sizes and attrition rates for virtual students")
```
...
kable(tab,digits=1,caption="Table 4: Population and sample sizes and attrition rates for in-person students")
```
...
The results for the three active control comparisons are in Table 5 and Figure 3. All of the differences are below the upper WWC threshhold of 0.25, though several are above the lower threshhold of 0.05, indicating the need for statistical adjustment. 
...
diffsTab <- lapply(diffs,
  ...
  kable(caption="Table 5: Cox indices (for binary or categorical covariates) and Hedges's g (for numeric covariates) comparing baseline covariate means between the Active Control and each of the experimental conditions",
        digits=3)
```
...
If attrition is ignorable, then the observed treatment groups in the analysis sample--i.e. non-attritor--should be comparble. 
Evidence against ignorable attrition could be expressed as evidence agains the null hypothesis 
$$H_0: X \perp\!\!\!\!\perp Z$$
